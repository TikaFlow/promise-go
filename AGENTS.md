...
- `eventloop.go`：`EventLoop` 及绝大多数公开 API —— 批量（`All`/`AllSettled`/`Race`/`Any`/`Some`）、迭代（`Map`/`Filter`/`Each`/`Reduce`）、定时器（`SetTimeout`/`SetInterval`/`ClearTimeout`/`ClearInterval`/`Delay`）、工具（`Async`/`Await`/`Try`/`Resolve`/`Reject`/`NewPromise`/`PromiseWithResolvers`/`QueueMicrotask`）、钩子注册（`On`/`Off`）、`Stop`
...
- `Then` 返回的新 Promise 状态由回调执行结果决定；回调返回 err → 新 Promise 被拒绝（规范 2.2.7.2）
