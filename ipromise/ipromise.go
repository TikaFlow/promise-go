...
// 当回调函数执行遇到错误时（意味着Then方法将会返回一个 Rejected Promise 实例），
// err仅代表出错，但应该将错误信息包含在v中（以支持任意类型），此时可以返回 (detail, summary) 格式的错误信息。
...
// 特别的，如果返回值是一个已拒绝的 Promise 实例，则以同样的理由拒绝新 Promise。
// 执行中报错，也将以同样的理由拒绝新 Promise。
