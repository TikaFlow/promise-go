...
// TestOnPromise 覆盖 EventLoop.OnPromise/OffPromise 全部分支。
//
// 注意：为免钩子影响其他测试，本测试使用独立的事件循环。
func TestOnPromise(t *testing.T) {
	t.Parallel()
	...
	t.Run("off-success", func(t *testing.T) {
		t.Parallel()
		el2 := StartEventLoop(1)
		defer el2.Stop()
	...
	t.Run("off-mismatch", func(t *testing.T) {
		t.Parallel()
		el2 := StartEventLoop(1)
		defer el2.Stop()
...
// TestOnPanic 覆盖 EventLoop.OnPanic/OffPanic 各 panic 事件分支。
//
// 注意：为免钩子影响其他测试，本测试使用独立的事件循环。
func TestOnPanic(t *testing.T) {
	...
	t.Run("hook-panic-cascade", func(t *testing.T) {
		t.Parallel()
		el2 := StartEventLoop(1)
		defer el2.Stop()
...
// 跨 Stop/flushTasks/TimerPanic 行为，保留为独立测试函数。
